...
A lightweight Cloudflare Worker that fetches multiple subscription URLs, merges their proxies, applies filters, and outputs a complete [Mihomo](https://github.com/MetaCubeX/mihomo) YAML config generated with a vendored copy of [AIsouler/MyClash](https://github.com/AIsouler/MyClash)'s `Script.js`.
...
### 3. Sync MyClash Logic

The Worker uses a committed local copy of MyClash `Script.js`, not a runtime remote fetch.
...
Review the generated diff in `src/vendor/myclash-script.ts` before deploying.
...
### MyClash Vendor (`src/vendor/myclash-script.ts`)

`src/vendor/myclash-script.ts` is generated from:

```
https://raw.githubusercontent.com/AIsouler/MyClash/main/Script/Script.js
```

The sync script downloads the upstream JavaScript and converts the bare `function main(config)` entrypoint into a typed ESM export that the Worker can import statically. Cloudflare deploys the committed local file, so production does not depend on `raw.githubusercontent.com` at request time.

Important limits:

- MyClash `Script.js` accepts an already parsed config object. It does not fetch subscription URLs or parse proxy URIs by itself.
- `clash-fusion` still owns subscription fetching, proxy parsing, filtering, deduplication, sorting, and YAML serialization.
- DNS and hosts inheritance only works when the source subscription is Clash YAML/JSON containing `dns` or `hosts`. URI-only subscriptions do not carry that metadata.
- Do not edit generated MyClash logic by hand unless you intentionally want to fork it. Prefer `pnpm sync:myclash` and review the diff.
...
4. **MyClash filtering** — The vendored MyClash script also removes common info nodes and direct/reject proxies before building groups
...
The DNS configuration and routing rules are generated by the vendored copy of [AIsouler/MyClash](https://github.com/AIsouler/MyClash)'s `Script.js` (see `src/vendor/`).
