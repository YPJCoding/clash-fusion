// This file is generated by scripts/sync-myclash.mjs.
// Source: https://raw.githubusercontent.com/AIsouler/MyClash/main/Script/Script.js
...
export function main(config: MyClashConfig): MyClashConfig {
  const newConfig: MyClashConfig = {};
  ...
  const functionalGroups: MyClashProxyGroup[] = [];
  ...
  const originalDnsConfig = asRecord(config.dns);
  ...
  // 提取节点域名对应的 DNS 配置
  const originalPolicyNameserver: Record<string, unknown> = {};
  for (const policy of [
  ...
  const originalHosts = asRecord(config.hosts);
  const proxyHosts: Record<string, unknown> = {};
